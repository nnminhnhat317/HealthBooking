...
def semanticize_csv_documents(documents):
    ...
    for doc in documents: # documents có định dạng List[Document] và Document( page_content="TenCoSo: ...\nDiaChi: ...", metadata={"row": 7557} )
        ...
        metadatas_csv.append({
            "source": "csv",
            "type": "medical_facility",
            "row": doc.metadata.get("row"),
            "TenCoSo": row.get("TenCoSo"),
            "TinhTrang": row.get("TinhTrang"),
            "DiaChi": row.get("DiaChi")
        })

    return semantic_texts, metadatas_csv
#
...
def ChangeCSVDocsFromOriginToLangChain(token_split_texts, metadatas):
    documents = []
    for i, text in enumerate(token_split_texts):
        doc = Document(
            page_content=text,
