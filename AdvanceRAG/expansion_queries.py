...
from utilsRAG import semanticize_csv_documents
...
csv = csvLoader.load()
semantic_texts, metadatas = semanticize_csv_documents(csv)
...
ids = [str(i) for i in range(len(token_split_texts))] # Tạo danh sách ID cho mỗi đoạn văn bản
# chroma_collection.add(ids=ids, documents=token_split_texts) # Nếu đã có dữ liệu trùng ID thì có thể ẩn dòng này đi, nếu có dữ liệu mới thì add lại
...
# chroma_collection.add(ids=idsCsv, documents=semantic_texts,metadatas=metadatas) # nếu đã có dữ liệu trùng ID thì có thể ẩn dòng này đi, nếu có dữ liệu mới thì add lại
if chroma_collection.count() == 0:
    chroma_collection.add(ids=idsCsv, documents=semantic_texts,metadatas=metadatas)
...
def rag_pipeline(query): # dự đoán đầu ra là -> tuple[str, list[Document]]:
    ...
    for doc_list in results["documents"]:
        retrieved_documents.extend(doc_list) # nối mảng - nối tất cả các chunk liên quan thành một MẢNG lớn
